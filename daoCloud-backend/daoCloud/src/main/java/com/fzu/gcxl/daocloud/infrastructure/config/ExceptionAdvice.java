...
public class ExceptionAdvice {
    ...
    public BaseResponse handle401(ShiroException e) {
        return new BaseResponse(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):" + e.getMessage(), null);
    }
    ...
    public BaseResponse handle401(UnauthorizedException e) {
        return new BaseResponse(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):当前Subject没有此请求所需权限(" + e.getMessage() + ")", null);
    }
    ...
    public BaseResponse handle401(UnauthenticatedException e) {
        return new BaseResponse(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):当前Subject是匿名Subject，请先登录(This subject is anonymous.)", null);
    }
    ...
    public BaseResponse handle401(CustomUnauthorizedException e) {
        return new BaseResponse(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):" + e.getMessage(), null);
    }
    ...
    public BaseResponse globalException(HttpServletRequest request, Throwable ex) {
        return new BaseResponse(this.getStatus(request).value(), ex.toString() + ": " + ex.getMessage(), null);
    }
