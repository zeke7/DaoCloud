package com.fzu.gcxl.daocloud.config.JWTPackage;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
		...
		System.out.println("过滤器拦截请求的入口方法");
		if (this.isLoginRequest(request, response)){
			System.out.println("判断是否是登录请求");
			return false;
		}
		boolean allowed = false;
		try {
			// 检测Header里的 JWT token内容是否正确，尝试使用 token进行登录
			System.out.println("检测Header里的 JWT token内容是否正确，尝试使用 token进行登录");
	...
	protected boolean isLoginAttempt(ServletRequest request, ServletResponse response) {
		System.out.print("isLoginAttempt:");
		System.out.println(((HttpServletRequest) request).getHeader(JwtUtils.AUTH_HEADER) == null);
	...
	protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
		...
		try {
			System.out.println("身份验证,检查 JWT token 是否合法");
	...
	protected boolean onLoginSuccess(AuthenticationToken token, Subject subject, ServletRequest request,
		...
		System.out.println("利用 JWT token 登录成功，会进入该方法");
